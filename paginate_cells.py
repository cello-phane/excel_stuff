...
for iter, n in enumerate(ranges_[::nth_rows*pages]):
    if divm_rows_uneven[1] > 0:
        if n+nth_rows >= numberofpages*nth_rows+divm_rows_uneven[1]:
            break
    for rownum in ranges_[n:n+nth_rows]:
        ...
        for p in range(pages):
            for col in nth_cols.split(','):
                ...
                col_iter+=1
        new_row+=1
tail_start_row = new_row
if divm_rows_uneven[1] > 0:
    for n in ranges_[nth_rows*numberofpages:nth_rows*numberofpages+divm_rows_uneven[1]]:
        row_values = [value for value in xlsfile_[ranges_[n]]]
        for col in nth_cols.split(','):
            colnum_ = int(col_to_n(str(col)))
            writesheet.write(tail_start_row, colnum_, str(row_values[colnum_]))
        tail_start_row+=1
